...
$category = 'festmenyek'; // ← ezt cseréld a megfelelő kategóriára

$stmt = $pdo->prepare("SELECT * FROM images WHERE category = ? AND visible = 1 ORDER BY position ASC");
    ...
    <tr>
      <td>
        <a href="../uploads/<?= $category ?>/<?= $img['filename'] ?>" class="lightbox-trigger">
          <img src="../uploads/festmenyek/<?= htmlspecialchars($img['filename']) ?>" alt="<?= htmlspecialchars($img['description']) ?>">
        </a>
      </td>
      <td>
